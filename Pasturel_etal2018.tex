...
\begin{itemize}
    \item Occurrence of a switch: $x_2^t \propto \Bb(1/\lambda)$
...
To sum up, the experimental protocol
is similar to that presented in~\seeFig{intro}-A, except for
this new generative model for the pseudo-random sequence $x_0^t$ of binary events. 
...
by a factor$~\rho \in [0, 1]$, relative to new information.
The value of the scalar $~\lambda$ represents
a compromise between responding rapidly
to genuine changes in the environment ($\rho \approx 1$) and
not prematurely discarding information still of value
for slowly changing contexts  ($\rho \approx 0$).
Equivalently, one can define $\tau = 1 / \rho$ as
...
% urn model ?
Ignoring the temporal occurrence of the switch, 
...
First, the trajectory $\hat{p}$ can serve as a regressor
...
along with the volatility of the environment
(e.g the duration since the occurrence of a switch).
% ------------------------------------------------------------------
...
we have computed the probability of each model parameterized by $r$
...
one instance of a sequence of simulated data
...
but results varied across participants~\seeApp{results_psycho}.
