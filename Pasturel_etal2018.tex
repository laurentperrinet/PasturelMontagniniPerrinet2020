...
to limit as fast as possible the behavioral impairment due
...
we should use a continuous range of probability bias $p$.
...
In particular, one first novelty of our study is that the variable $p$ is itself a random variable.
...
one may introduce a parametric mechanism controlling for the volatility.
...
To sum up, \seeEq{bsm} defines the binomial switching model 
...
In~\seeSec{inter}, we will analyze the inference of the volatility
