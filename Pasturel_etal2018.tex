...
To extend such results to more generic experimental conditions,
we have modified the experimental protocol of~\citep{Montagnini2010} in three aspects.
...
we have used a continuous range of probability bias $p$ 
...
we have extended the protocol such that the probability bias is fixed in sub-blocks,
and that the duration of these blocks is drawn randomly with given statistics.
The equations for this protocol will be detailed below~(\seeSec{bayesian_change_point}).
Last, we replicated part of those results by repeating the same protocol
in a psychophysical judgement task in a day subsequent to the recording of eye movements.
In this task, we asked to the participants,
to evaluate their confidence for the next trial 
