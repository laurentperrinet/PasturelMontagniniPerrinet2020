...
is that of a binomial process knowing an updated sample size of $r+1$
and an updated probability bias of $\frac{\mu^{(r)}_{t} \cdot r + o}{r+1}$.
...
with $Z$ such that $\Ll(o=1| \mu\mu^{(r)}_{t}, r) + \Ll(o=1 | \mu\mu^{(r)}_{t}, r)=1$. % where
%\eq{
%Z = {(\mu \cdot \nu + 1)}^{\mu \cdot \nu + 1}  \cdot {((1- \mu)\cdot \nu )}^{(1- \mu)\cdot \nu }  +
%    {(\mu \cdot \nu )}^{\mu \cdot \nu }  \cdot {((1- \mu)\cdot \nu + 1)}^{(1- \mu)\cdot \nu + 1}
%}
The derivation of this function is detailed in~\seeApp{likelihood}. Finally, we use that function whenever we observe the new datum : $\pi^{(r)}_t = \Ll(x_0^{t+1}| \mu^{(r)}_{t}, r)$
