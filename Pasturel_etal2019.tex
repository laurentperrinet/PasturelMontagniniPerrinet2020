...
\newcommand{\Abstract}{
Animal behavior must constantly adapt to changes, for example when the state of an environmental context changes unexpectedly. For an agent that interacts with this volatile setting, it is important to react accurately and as quickly as possible. It has already been shown that when a random sequence of directions of motion (to the right or left) of a visual target is biased to one direction, human observers adapt to accurately anticipate the expected direction with their eye movements. Here, we prove that this ability extends to a volatile environment where probability biases could change at random switching times. In addition, we also recorded the explicit direction prediction, along with the relative confidence, as reported by human observers. These results were compared to the estimates of a probabilistic agent that is optimal in relation to the event switching generating model. Compared to other models such as the leaky integrator, we found a better match between our probabilistic agent and the behavioral responses, both for the implicit (anticipatory eye movements) and the explicit ones. Furthermore, we were able to fit the individual experimental data with different levels of switching volatility in the model and derive a common marker for the inter-individual variability of participants, by titrating their level of preference between exploration and exploitation. These results prove that in such an unstable environment, human observers can still effectively represent an internal belief about the environmental contingencies, and use this representation both for sensory-motor control and for explicit judgments. This work offers an innovative approach to more generically test human cognitive abilities in uncertain and dynamic environments.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
of the underlying statistical process: these coincidences which drive patients to the doctor, but
they are simply 
instances drawn from a stationary random process.
...
at the time scale of several seconds to minutes \AM{(or hours in the case of sensorimotor persistent adaptation)}.
...
\AM{I SUGGEST TO ANTICIPATE THIS HERE INSTEAD OF AT THE END Interestingly, priming effects at cognitive levels
...
to introduce latent variables which formalize how different hypotheses predict \AM{these? RATHER EXPERIMENTAL?} measurements.
Each stated hypothesis is quantitatively formalized
by defining a graph of \AM{probabilistic} dependencies between specific variables
using a generative model for the \AM{CAN WE CUT THIS? input / output response function
