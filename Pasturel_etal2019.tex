...
This scalar is returned by the binary function $\pi^{(r)}_t = \Ll(x_0^{t+1}| \mu^{(r)}_{t}, \nu^{(r)}_{t})$.
whenever we observe the new datum
and where this binary function $\Ll(o| p, r)$ evaluates at each node the likelihood of each alternative outcome $o=1$ or $o=0$
...
\eql{
\Ll(o| p, r) = \frac{1}{Z} \cdot {(p \cdot r + o)}^{p \cdot r + o} \cdot {((1- \mu^{(r)}_{t})\cdot r + 1- o)}^{(1- p)\cdot r + 1- o}
\label{eq:likelihood}
}
with $Z$ such that $\Ll(o=1 p, r) + \Ll(o=1 | p, r)=1$. % where
%\eq{
%Z = {(\mu \cdot \nu + 1)}^{\mu \cdot \nu + 1}  \cdot {((1- \mu)\cdot \nu )}^{(1- \mu)\cdot \nu }  +
%    {(\mu \cdot \nu )}^{\mu \cdot \nu }  \cdot {((1- \mu)\cdot \nu + 1)}^{(1- \mu)\cdot \nu + 1}
%}
