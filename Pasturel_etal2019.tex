...
\node [anchor=north west]  (imgA) at (0.000\linewidth,.618\linewidth){\includegraphics[width=0.300\linewidth]{1_A_Experiment_randomblock}};
\node [anchor=north west]  (imgB) at (0.290\linewidth,.618\linewidth){\includegraphics[width=0.500\linewidth]{1_B_protocol_recording}};
...
\draw [anchor=north west] (0.290\linewidth, .62\linewidth) node {$\mathsf{(B)}$};
...
by using a simple, 3-layered generative model of fluctuations in target directions.
This binary variable is chosen using a Bernouilli trial of a given probability bias.
This bias is itself chosen at random from a given prior 
...
with a given hazard rate (here on average once every $40$ trials).
\textbf{(B)}~
