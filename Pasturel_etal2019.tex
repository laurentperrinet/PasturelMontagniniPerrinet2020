...
\sum_{r^{t}} \pi^{(r)}_t \cdot \beta^{(r)}_t
\label{eq:pred}
}
where $\pi^{(r)}_t=Pr(x_0^{t+1} | r^{t}, x_0^{0:t})$ is the likelihood of a future datum $x_0^{t+1}$ 
conditioned on the run-length and 
$\beta^{(r)}_t=Pr(r^t | x_0^{0:t})$ is the probability of each possible run-length.
...
$\beta^{(r)}_t$ at each node.
In particular, at the occurrence of the first trial (for the node $r^0=0$)
there is a switch and beliefs are set to initial values $\beta^{(0)}_0=Pr(r^t=0)=1$ and 
...
\eqa{
\beta^{(r)}_t \propto \sum_{r^{t-1} \geq 0}  Pr(r^t | r^{t-1}) \cdot  \pi^{(r)}_{t-1} \cdot  \beta^{(r)}_{t-1}
\label{eq:pred_node}
}
...
\eqa{
\beta^{(r+1)}_t = \frac{1}{B} \cdot \beta^{(r)}_{t-1} \cdot \pi^{(r)}_{t-1} \cdot (1-h)
\label{eq:beta_noswitch}
}
(where $h$ is the scalar defining the hazard rate)
but also the change-point probabilities:
\eqa{
\beta^{(0)}_t  = \frac{1}{B} \cdot \sum_{r \geq 0} \beta^{(r)}_{t-1} \cdot \pi^{(r)}_{t-1} \cdot h
...
by computing the likelihood $\pi^{(r)}_t$ of a future datum 
...
%-------------------------------------------------------------%
%: 2Bc online estimation: run-lengths
%-------------------------------------------------------------%
...
\eqa{
& \mu^{(r+1)}_{t} = \frac{r}{r+1} \cdot \mu^{(r)}_{t-1} + \frac{1}{r+1} \cdot x_0^{t-1} \\
& \nu^{(r+1)}_{t} = \nu^{(r)}_{t-1} + 1
}
...
%-------------------------------------------------------------%
We can now detail the computation of the likelihood of a future datum $x_0^{t+1}$ with respect to
the current beliefs : $\pi^{(r)}_t = Pr( x_0^{t+1} |  \mu^{(r)}_{t}, \nu^{(r)}_{t})$. %}
This binary function evaluates at each node the likelihood of each alternative outcome of $x_0^{t+1}$
(two hypothesis $o=1$ or $o=0$)
knowing the mean bias $\mu^{(r)}_{t}$ 
and conditioned on the run-length $r$. 
For each outcome, the likelihood of observing an occurrence of $o$ at the next trial,
is the probability of a binomial random variable knowing 
an updated probability bias of $\frac{\mu^{(r)}_{t} \cdot r + o}{r+1}$, 
a number $\mu^{(r)}_{t} \cdot r + o$ of trials going to the right and
a number $(1-\mu^{(r)}_{t}) \cdot r + 1 - o$ of trials to the left.
After some algebra, this defines the likelihood as :
\eql{
\Ll(o| \mu^{(r)}_{t}, r) = \frac{1}{Z} \cdot {(\mu^{(r)}_{t}\cdot r + o)}^{\mu^{(r)}_{t}\cdot r + o} \cdot {((1- \mu^{(r)}_{t})\cdot r + 1- o)}^{(1- \mu^{(r)}_{t})\cdot r + 1- o}
\label{eq:likelihood}
}
with $Z$ such that $\Ll(o=1| \mu^{(r)}_{t}, r) + \Ll(o=1 | \mu^{(r)}_{t}, r)=1$. % where
%\eq{
%Z = {(\mu \cdot \nu + 1)}^{\mu \cdot \nu + 1}  \cdot {((1- \mu)\cdot \nu )}^{(1- \mu)\cdot \nu }  +
%    {(\mu \cdot \nu )}^{\mu \cdot \nu }  \cdot {((1- \mu)\cdot \nu + 1)}^{(1- \mu)\cdot \nu + 1}
%}
The derivation of this function is detailed in~\seeApp{likelihood}. Finally, we use that function whenever we observe the new datum as the scalar returned by the binary function $\pi^{(r)}_t = \Ll(x_0^{t+1}| \mu^{(r)}_{t}, r)$.

%-------------------------------------------------------------%
%: 2Be online estimation: prediction
%-------------------------------------------------------------%
Finally, the agent may infer at each trial the belief
...
(see Equations~\ref{eq:pred},~\ref{eq:beta_noswitch},~\ref{eq:beta_switch} and~\ref{eq:likelihood})
