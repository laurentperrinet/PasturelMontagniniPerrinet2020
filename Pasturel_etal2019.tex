...
To summarize, the algorithm that we presented is an implementation of the  ``Bayesian Online Changepoint Detection''~\citet{AdamsMackay2007}
extended for the class of binary inputs.
In this appendix, we start by some basic concepts, detail some intermediate steps and highlight some key differences with their implementation. We also provide with a python implementation which is openly available at \url{https://github.com/laurentperrinet/Bayesianchangepoint}.


\subsubsection{The binomial and Beta distributions}
Let us as a start define some basic concepts. A Bernoulli trial is the outcome of a binary random variable $x$ knowing a probability bias $p$ and can be formalized as:
\eq{
p(x | p) = p^x \cdot (1-p)^{1-x}
}


Then, the binomial distribution is defined as the probability that the sum of $n$ independent Bernoulli trials is $k$:
\eq{
\Pr(k;n,p) = \Pr(X = k) = {n\choose k}p^k(1-p)^{n-k}
}

Knowing such a model for $X$ it can be of interest to find an estimate of the parameter of the Bernoulli trial, that is of the probability bias $p$. This distribution function is called the conjugate of the binomial distribution which is the Beta-distribution. For example, the beta distribution can be used in Bayesian analysis to describe initial knowledge concerning probability of success such as the probability that a product will successfully complete a stress test. The beta distribution is a suitable model for the random behavior of percentages and proportions.
...
and inversely, $\alpha + \beta = r$ and $p = \frac{\alpha}{\alpha +\beta} = 1- \frac{\beta}{\alpha + \beta}$
