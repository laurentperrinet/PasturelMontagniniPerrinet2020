...
\node [anchor=north west]  (imgA) at (0.000\linewidth,.628\linewidth){\includegraphics[width=0.250\linewidth]{1_B_Trace_moyenne}};
\node [anchor=north west]  (imgB) at (0.250\linewidth,.618\linewidth){\includegraphics[width=0.500\linewidth]{1_B_protocol_recording}};
\node [anchor=north west]  (imgC) at (0.844\linewidth,.538\linewidth){\includegraphics[width=0.156\linewidth]{1_C_protocol_bet_simple}};
\draw [anchor=north west] (0.000\linewidth, .62\linewidth) node {$\mathsf{(A)}$};
\draw [anchor=north west] (0.250\linewidth, .62\linewidth) node {$\mathsf{(B)}$};
...
\emph{Behavioral experiments: tracking eye movements and confidence rating in a volatile environment}
\textbf{(A)}~
We tested the capacity of human subjects to adapt to a volatile environment 
by using a simple, 3-layered generative model of fluctuations.
At the lowest layer, a binary variable is chosen using a Bernouilli trial of a given probability bias.
This bias is chosen from a given prior and is constant for as many trials until a switch is generated.
Switches are generated in the third layer as a Bernouilli trial with a given hazard rate.
\textbf{(B)}~
...
to the environmental volatility of motion direction at the conscious level,
we invited each observer to perform a new variant of the direction-biased experiment on a subsequent day,
where, instead of recording their eye movements, 
we asked participants to rate, before each trial, the level of confidence
