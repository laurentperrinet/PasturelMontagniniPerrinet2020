...
the transient disappearance of a moving target~\citep{Badler2006,BeckerFuchs1985} \AM{REF OrbandeXivry}.
...
to pick up some regularity in the properties of visual motion, such as speed \AM{REF OrbandeXivry} or direction \AM{REF Yang, Darlington}.
...
before target motion onset~\citep{Kowler1979a,Kowler1979b, Kowler1984}. \AM{Merge citations?}. %~(see \seeFig{intro}-A).
...
we invited each observer to perform a new variant of the direction-biased experiment on a \textbf{subsequent day},
\CP{ATTENTION parie etait fait AVANT l'enregistrement occulaire}
...
\CP{seance 1 : paris, seance 2 : enregistrement}
\textbf{While in a first session, we recorded anticipatory eye movements~(see \seeFig{intro}-B),
...
\node [anchor=north west]  (imgB) at (0.36\linewidth,.585\linewidth){\includegraphics[width=0.64\linewidth]{3_BCP_readouts}};
\draw [anchor=north west] (0.000\linewidth, .62\linewidth) node {$\mathsf{(A)}$};
...
while red lines \CP{Lignes grises} stand for the possibility that a switch happened:
...
%which
We then assume the knowledge of the predictive probability conditioned on the run-length,
...
$\beta^{(r)}_t=Pr(r^t | x_0^{0:t})$ is the probability distribution of each possible run-length given the observed data.
...
In the graph, \AM{Not clear yet... \seeEq{pred_node} corresponds to a message passing from the nodes at time $t-1$
...
This allows us to compute the \emph{growth probability} for each run-length $r \geq 0$,
that is, the belief on an incremented run-length at the next trial, before observing a new datum:
\eqa{
\beta^{(r+1)}_t = \frac{1}{B} \cdot \beta^{(r)}_{t-1} \cdot \pi^{(r)}_{t} \cdot (1-h)
\label{eq:beta_noswitch}
}
where $h$ is the scalar defining the hazard rate. It also allows to express the change-point probability as:
\eqa{
...
that is, knowing there was a sequence of $r^t$ Bernoulli trials with a fixed bias in that epoch.
...
with $Z$ such that $\Ll(r|p, o=1) + \Ll(r|p, o=0)=1$. % where
%\eq{
%Z = {(\mu \cdot \nu + 1)}^{\mu \cdot \nu + 1}  \cdot {((1- \mu)\cdot \nu )}^{(1- \mu)\cdot \nu }  +
%    {(\mu \cdot \nu )}^{\mu \cdot \nu }  \cdot {((1- \mu)\cdot \nu + 1)}^{(1- \mu)\cdot \nu + 1}
%}
The full derivation of this function is detailed in~\seeApp{likelihood}.
This provides us with that the likelihood as $\pi^{(r)}_t = \Ll(\nu^{(r)}_{t-1} | \mu^{(r)}_{t-1}, x_0^{t})$.
...
This highlights that contrary to the leaky integrator, % for which the inference $\hat{x_2}=h$ was fixed,
...
\seeFig{bayesianchangepoint}-A shows a graph-based representation of the BBCP estimate of the run-length for one instance of a short sequence \textbf{($10$ trials)} \CP{($15$ ou $14$ trials)} of simulated data $x_0^t$
of leftward and rightward trials, with a switch from a \textbf{low} \CP{0.5} to a \textbf{high} \CP{0.9} probability
...
\seeFig{bayesianchangepoint}-B, upper panel, illustrates the predicted probability $\hat{x_1}^t$, as well as the corresponding uncertainty (the shaded areas correspond to $.05$ and $.95$ quantiles) when
...
\textbf{As for the synthetic example above}  \CP{pas dans \seeFig{bayesianchangepoint}-A},
...
as our model \AM{I WOULD CHANGE THIS: integrates belief about a current phase during two switches INTO THIS: best estimate of the current length is steadily incremented by $1$ between two switches, and the probability $\hat{x_1}^t$ is estimated  by integrating sensory evidence in this epoch (it 'stays')}.
...
we computed \AM{the negative log-likelihood (in bits) of the estimated probability bias,
knowing the real probability - isn't H the joint entropy and C something related to the mutual information? H and C should both be defines},
...
than for the leaky integrator \CP{($\Cc = XXXX \pm XXX$)}, showing it provides overall with a better prediction.
...
\textbf{averaged over all observers, for each individual observer or independently in all individual sub-blocks.}\CP{}
In a second step, by testing different values of $h$ assumed by the agent
but for a fixed hazard rate $h=1/40$ in the BSM,
we found that \AM{this cost - the cost has not been defined: the KL distance?} can be computed for a range of hypothesized values of $h$ 
...
of the probability bias $p$ (blue line) \CP{ligne bleu plus \'epaisse pour la rendre plus visible ?}
...
In a \textbf{first session} \CP{session 2}, we recorded the participants' eye movements and
extracted the aSPEM velocity (for details, see \seeApp{em}).
Among our $12$ participants, in~\seeFig{results_psycho} we show the aSPEM velocity of two representative subjects throughout a sub-block of $200$ trials of the experimental sequence.
...
(TD, leftward or rightward) of the Bernoulli trials, whereas in the two bottom panels, we compare the evolution of the recorded aSPEM (grey line) with
the real value of the hidden probability bias $x_1$ (step-like blue \textbf{shaded} \CP{il n'y a plus d'ombrage pour $x_1$} curve),
and the value inferred using the BBCP model (green line), along with the \CP{il manque un mot !} (green shaded area).
...
provided by the \textbf{second} \CP{premi\`ere} experiment
...
Moreover, we observe again that a stronger bias leads 
...
\CP{aSPEM / BCP : ($r = 0.657$ $MI = 0.687$) pour les sujet - ($r = 0.6734 \pm 0.0797 $ $MI =  0.7079 \pm 0.1341$) notebook 4}
...
for the real probability ($r = XXX \pm XXX$, $MI = XXX \pm XXX$, $p=XXX$\footnote{All following $p$-values are obtained from the Wilcoxon signed-rank test.}) % https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.wilcoxon.html
\CP{aSPEM / Real : ($r = 0.597$ $MI = 0.529$) pour les sujet - ($r = 0.6138 \pm 0.0698 $  $p=0.0022$ - $MI = 0.5622 \pm 0.1079$ $p=0.0022$)}
...
-- real probability : ($r = 0.679$ $MI = 0.795$) pour les sujet - ($r = 0.6949 \pm 0.0861 $ $p=0.0022$ - $MI =  0.9400 \pm 0.2550$ $p=0.0022$) }
...
($r = XXX \pm XXX$, $MI = XXX \pm XXX$, $p=XXX$).
\CP{SANS PAUSE -- aSPEM / BCP :($r = 0.651$ $MI = 0.686$) pour les sujet - ($r = 0.6678 \pm 0.0782 $ $MI =  0.7123 \pm 0.1250$
/ Leaky :($r = 0.525$ $MI = 0.561$) pour les sujet - ($r = 0.5480 \pm 0.0749 $ $p=0.0037$ - $MI =  0.5777 \pm 0.0961$ $p=0.0037$ )
/ real probability : ($r = 0.597$ $MI = 0.529$) pour les sujet - ($r = 0.6138 \pm 0.0698 $ $p=0.0028$ -  $MI =  0.5622 \pm 0.1079$ $p=0.0022$) }
...
\textbf{Also, we have shown that knowing an observed sequence of behavioral responses,
we could fit the best value of $\tau$ which would explain it
with respect to the computed value of the mutual information.} \CP{coefficient de correlation ?} \CP{traduction tr\`es bizzar }
Thus, by extracting the best-fit parameters for every participant and experimental session
we can expect to \textbf{characterize inter individual differences} \CP{pas r\'eusit \`a caract\`eriser les sujets} and the covariation of these across response modalities.
%-------------------------------------------------------------%
...
each individual is represented by a \textbf{different color} \CP{les 3 valeurs de h de chaque sujets sont relier pas des ligne pointill\'e et les couleurs des point correspond au 3 blocs}.
...
This plot shows that best fit hazard rates are in general higher than the ground truth (\textbf{black line}), \CP{ligne bleu}
\textbf{that there is a positive correlation between both experiments around the identity (dashed line),}
\CP{pas de bonne correlation :/}
\textbf{and finally that for each individual these values cluster pretty much together.}\CP{Pas de regroupement}
...
\CP{Histogramme pour tous les sujets, et pour chaque block + ligne verticale et horizontal pointill\'e pour m\'ediane pour chaque block et pour tous + ligne pleine -> score max pour BBCP, m\^eme profile que $h_{va}$ mais diff\`erent de $h_{bet}$}
 }
...
We computed the\textbf{ mutual information} \CP{coefficient de correlation}
%for $20$ equally spaced values of $h$ between $0.01$ and $0.1$
%for $20$ geometrically spaced values of $h$ between $0.01$ and $0.1$
for the BBCP-model responses of \textbf{$21$ geometrically spaced values of $h$ between $0.0025=1/400$ and $0.25=1/4$} \CP{1600 valeurs de $\tau$ entre 1 et 1601}.
We then extracted different estimates of $h_A$ and $h_R$,
respectively for aSPEM and the rating scale,
by choosing the hazard rate values corresponding to a \textbf{maximal mutual information} \CP{coefficient de correlation maximal}.
...
These provide with $11$ \CP{$3$} values of the best fitted hazard rate for each session and observer.
...
Overall, the value of \textbf{mutual information} \CP{Coefficient de correlation} of the best fit hazard rate
was higher than that computed in~\seeFig{results_psycho}
with respectively $\text{MI} = XX~ \pm XX~$ \CP{$\text{R} = XX~ \pm XX~$ a calculer !}  for the first session and
$\text{MI} = XX \pm XX$ \CP{$\text{R} = XX~ \pm XX~$ a calculer !}  for the second.
...
(with $h_A = XX \pm XX$ \CP{a calculer !} for the first session and
$h_R = XX~\ms \pm XX$ \CP{a calculer !} for the second),
...
that we could detect, to some degree, \textbf{individual differences in the choice behavior} \CP{pas de difference individuel}
between exploration and exploitation.
%-------------------------------------------------------------%
%: 4C analysis
%-------------------------------------------------------------%
\textbf{Finally, there is a  consistent
...
between exploration and exploitation. } \CP{pas de correlation entre les deux session}

\CP{
\begin{itemize}
\item  diff\`erence signification entre le block 3 et les deux autres blocs
\item  $h_{bet}$ plus bas et plus disperc\'e que $h_{va}$ (pour la m\'ediane) notebook 5
\item  sujet qui se trouve a dans les limite de h
\end{itemize} }
...
\item Finally, we found that the experimental data of each different participant matched to different levels of belief's precision on the volatile environment, some being more or less conservative. \textbf{Interestingly, the individual level was relatively coherent across the two experiments (namely for the unconscious anticipatory eye movements and the conscious rating). }\CP{pas de corr\'elation entre les deux h}
\end{itemize}
%: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The paradigm included two experiments performed on \textbf{two consecutive days}\CP{pas pour tout les sujets certain ont pass\'e la deusi\`eme exp\`erience une semaine apr\`es et je crois qu'il y en un qui a fait les deux le m\^eme jour (\`a v\'erifier !)'} by each participants and involving the presentation of the same sequence of trials, while collecting a different behavioral response, \textbf{the \textit{Eye movements} and the \textit{Explicit prediction} (or \textit{Bet}) experiments }\CP{\textit{Explicit prediction} avant\textit{Eye movements} }. Asked after the experiment, no observer noticed that the same (pseudo-)random sequence of target directions was used in both experiments.
...
%
%
%* adapted from https://github.com/JackKelly/bayesianchangepoint by Jack Kelly (2013) for a binary input.
%
