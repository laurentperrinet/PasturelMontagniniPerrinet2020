...
with $Z$ such that $\Ll(p, r|o=1) + \Ll(p, r|o=0)=1$. % where
%\eq{
%Z = {(\mu \cdot \nu + 1)}^{\mu \cdot \nu + 1}  \cdot {((1- \mu)\cdot \nu )}^{(1- \mu)\cdot \nu }  +
%    {(\mu \cdot \nu )}^{\mu \cdot \nu }  \cdot {((1- \mu)\cdot \nu + 1)}^{(1- \mu)\cdot \nu + 1}
%}
...
Second, we may use this information to read-out the most likely probability-bias and 
...
\Cc &= \sum_t H(x_1^t, \hat{x_1}^t ) - H(x_1^t, x_1^t )
\label{eq:KL}
}
...
than for the leaky integrator ($\Cc = XXXX \pm XXX$), showing it provides with a better prediction.
...
we decided to keep this parameter fixed for any agent, but evaluate
how well it matches to the experimental outcomes at the different scales of the protocol:
in sub-blocks, for each individual observer or averages over all observers.
...
occurring every $50$ trials (as denoted by vertical black lines, see main tex), 
...
the increase of the uncertainty after each multiple of $50$ trials.
...
(TD, leftward or rightward) of the Bernoulli trial, whereas in the two bottom panels, we compare the evolution of the recorded aSPEM (black line) with
...
(respectively $p=XXX$ and $p=XXX$ for $r$ and  $p=XXX$ and $p=XXX$ for MI).
}
...
for the real probability ($r = XXX \pm XXX$, $MI = XXX \pm XXX$, $p=XXX$)
...
we added the assumption %for both models, 
that there was a switch at each pause. 
However, removing this assumptions did not significantly change the match of the model
...
First, by looking at the experimental measures for two representative participants in~\seeFig{results_psycho},
supporting the qualitative match between the behavioral data and the model predictions,
...
for $21$ geometrically spaced values of $h$ between $0.0025$ and $0.25$.
...
by shifting a $100$-trials window by steps of $50$ trials.
...
with respectively $MI = XX~ \pm XX~$ for the first session and
$MI_R = XX \pm XX$ for the second.
A part of the variability in the the estimated hazard rates comes 
...
(with $h_A = XX \pm XX~\Hz$ for the first session and
