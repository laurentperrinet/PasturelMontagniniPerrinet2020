...
%which 
Using that latent variable, one computes the marginal predictive distribution:
\eql{
Pr(x_0^{t+1} | x_0^{0:t}) = 
%\sum_{r^{t}} \beta^{(r)}_t \cdot Pr(x_0^t | r^{t}, x_0^{0:t-1}) 
\sum_{r^{t}} Pr(x_0^t | r^{t}, x_0^{0:t-1}) \cdot Pr(r^t | x_0^{0:t})
...
Second, for the prediction step, 
we assigns a probability strength at each node
...
\eqa{
\beta^{(r)}_t \propto \sum_{r^{t-1}}  Pr(r^t | r^{t-1}) \cdot  \pi^{(r)}_t \cdot  \beta^{(r)}_{t-1}
\label{eq:pred_node}
}
...
we can therefore make a \emph{prediction} for our belief of the probability bias at the next trial $t+1$,
prior to the observation of a new datum $x_0^{t+1}$ and resume the update / prediction cycle.
...
By recurrence (see \seeFig{bayesianchangepoint}-A), one can show that at any trial $t>0$,
...
Note that $\mu^{(r+1)}_{t}$ is a moving average at trial $t$ on the $r$ last samples
before observing the new datum,
...
%-------------------------------------------------------------%
We can now detail the computation of the likelihood of this datum $x_0^t$ with respect to
the current beliefs : $\pi^{(r)}_t = \Ll( x_0^t |  \mu^{(r)}_{t}, \nu^{(r)}_{t})$. %}
In all generality, to perform this computation for some binary data $o$,
we have computed the likelihood of each alternative outcome
(the two hypothesis $o=1$ or $o=0$)
knowing a Beta distribution pdf at each node
which is parameterized by its mean $\mu$ and the sample size $\nu$.
This defines the likelihood
$\Ll(o | \mu, \nu) = \frac{1}{Z} Pr(o |\mu, \nu)$
with $Z$ such that $\Ll(o=1| \mu, \nu) + \Ll(o=1 | \mu, \nu)=1$.
It measures the relative frequency of observing an occurrence of $o$ at trial $t$,
as the updated estimation of the probability bias of $\frac{\mu \cdot \nu + o}{\nu+1}$
knowing a sample size of $\nu+1$.
Following the formulation of the Beta probability distribution, it follows:
\eql{
\Ll(o | \mu, \nu) = \frac{1}{Z} \cdot {(\mu\cdot \nu + o)}^{\mu\cdot \nu + o} \cdot {((1- \mu)\cdot \nu + 1- o)}^{(1- \mu)\cdot \nu + 1- o}
\label{eq:likelihood}
}
where
\eq{
