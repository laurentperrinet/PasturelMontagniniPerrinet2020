...
In particular, at the occurrence of the first trial there is a switch and 
beliefs are set to initial values $\beta^{(0)}_0=Pr(r^t=0)=1$ and 
...
with $B$ such that $\sum_{r \geq 0} \beta^{(r)}_{t} = 1$. Note that $\beta^{(0)}_t=h$ and thus $B=\sum_{r \geq 0} \beta^{(r)}_{t-1} \cdot \pi^{(r)}_{t-1}$.
