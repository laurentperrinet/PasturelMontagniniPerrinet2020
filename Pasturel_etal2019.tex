...
in which human observers were presented with several $500$ trials blocks of horizontal target motion with a block-dependent direction probability bias, and they were asked to track the target with their gaze.} % TODO:check nnumbers
The eye-movements task was an adapted version of a task developed by ~\citet{Montagnini2010}. Each one of $600$ trials consisted of sequentially:
...
~\textbf{(A)} This plot shows a synthesized sequence of events that correspond to a binomial choice, either $0$ or $1$, which will then correspond to a leftward or rightward movement of the target.
Here, the probability switched at trial $5$ from a value of $.25$ to $.75$.
...
but where we switched the probability at trial $100$ from a value of $.25$ to $.75$,
we show
