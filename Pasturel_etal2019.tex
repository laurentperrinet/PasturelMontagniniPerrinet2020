...
This scalar is returned by the binary function $\pi^{(r)}_t = \Ll( \mu^{(r)}_{t}, \nu^{(r)}_{t} | x_0^{t+1})$.
whenever we observe the new datum
and where this binary function $\Ll(p, r | o)$ evaluates at each node the likelihood of each alternative outcome $o=1$ or $o=0$
knowing a mean bias $p=\mu^{(r)}_{t}$ 
and a sample size $r=\nu^{(r)}_{t}$. 
...
ENVIRONMENTAL REGULARITIES, COGNITIVE PROCESSES AND VISUAL PERCEPTION
...
WHAT STATISTICS IS LEARNED IN STATISTICAL LEARNING?
...
in some pathological disorders such as schizophrenia~\citep{Adams12, JardriDeneve REF}.

COMPUTATIONAL PHENOTYPING OF HUMAN PARTICIPANTS
