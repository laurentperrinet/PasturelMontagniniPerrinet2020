...
which computes the marginal predictive distribution:
\eql{
Pr(x_0^{t+1} | x_0^{0:t}) = \sum_{r^{t}} Pr(x_0^t | r^{t}, x_0^{0:t-1}) \cdot \beta^{(r)}_t %= \sum_{r^{t}} Pr(x_0^{t+1} | x_0^{(r^{t})}) \cdot \beta^{(r)}_t
\label{eq:pred}
}
...
we use a knowledge of $\beta^{(r)}_{t-1}$ at trial $t-1$ and
...
%-------------------------------------------------------------%
Indeed, the only algorithmic difference with the BCP model~\citep{AdamsMackay2007},
...
one can compute the likelihood :
$
