...
In this appendix, we start by some basic concepts, detail some intermediate steps and highlight some key differences with their implementation. We also provide with a python implementation which is openly available on \href{https://github.com/laurentperrinet/Bayesianchangepoint}{GitHub}.
...
To quantitatively evaluate our results with respect to another probability bias, we computed in~\seeEq{KL} the cost as the Kullback-Leibler divergence  $\KL{\hat p}{p}$ between samples $\hat p$ and model $p$ under a Bernoulli distribution:
\begin{equation}
