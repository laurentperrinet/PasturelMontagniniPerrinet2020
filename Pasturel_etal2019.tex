...
%: 1A : cognitiove adaptation to volatility; general volatility and perceptual learning
...
``unlucky'' coincidences that originate from \AM{I THINK WE HAVE TO DEFINE VOLATILITY TO START WITH, TO EXCLUDE CONFUSION} the volatility
...
at the time scale of one to several minutes. \AM{RATHER seconds?}
...
how current stimuli are perceived~\citep{Sotiropoulos2011, Adams12} and 
...
\node [anchor=north west]  (imgB) at (0.400\linewidth,.600\linewidth){\includegraphics[width=0.225\linewidth]{1_B_protocol_recording}};
\node [anchor=north west]  (imgC) at (0.700\linewidth,.600\linewidth){\includegraphics[width=0.225\linewidth]{1_C_protocol_bet_simple}};
\draw [anchor=north west] (0.000\linewidth, .62\linewidth) node {$\mathsf{(A)}$};
\draw [anchor=north west] (0.360\linewidth, .62\linewidth) node {$\mathsf{(B)}$};
\draw [anchor=north west] (0.670\linewidth, .62\linewidth) node {$\mathsf{(C)}$};
\end{tikzpicture}
}
...
\AM{By definition, volatility measures the temporal variability
...
to extract an inference of the \AM{time-varying} agent's belief about volatility~\citep{Vossel14}.
...
Indeed, within each sub-block, the volatility \AM{? RATHER: UNCERTAINTY ABOUT?} of the value of $p$
