...
for two representative observers.
We have overlaid 
(1) the evolution of the value 
of the probability bias $p$ used to generate the TD sequence (red shaded areas)
and which are hidden to the observers
(2) the recorded aSPEM strength as measured by the horizontal eye velocity estimated right before
the onset of the visually-driven SPEM (black line);
these values were here scaled according to their extremal values,
(3) the bet score as given by the rating scale (red line),
(4) the results of the BBCP model (blue line).
...
%: <----- Laurent is here ----->
%-------------------------------------------------------------%
%: 3Aa raw results show a qualitative trend
%-------------------------------------------------------------%
...
(TD, leftward or rightward)
the true value of the true (hidden) value $x_1$
...
consistent with the dependence of the variance of a Bernouilli trial
...
by comparing human observers to the ideal observers we defined.and the value of the model-inferred (predicted) probability bias
