...
as compared to a random choice, as reported previousky ~\citep{Meyniel15}. %\AM{statistics to cite for this?}
...
we could fit the best value of $h$ which would explain the observations
with respect to the pearson coefficient ot the mutual information.
Thus, by extracting the best-fit parameters for each participant and experimental session
we can expect to better understand the variety of inter individual differences. % and the covariation of these across response modalities.
%-------------------------------------------------------------%
...
The dots correspond to independent estimates of the hazard rate in each $200$ trial sub-block and
each individual subject are joined by dotted lines.
...
Note that the histograms (gray shaded areas) is much more narrower 
for the eye movement session than for the bet experiment,
as shown by the evolution of the cumulative distributions (plain lines in black or colors).
...
For $1600$ linearly spaced values of $\tau$ from $1$ to $1600$ trials,
we computed the correlation coefficient with the BBCP-model responses parameterized 
by the modified value fo the hazard rate $h = \frac 1 \tau$.
We then extracted different estimates of $h_A$ and $h_R$,
respectively for aSPEM and the rating scale,
by choosing the hazard rate values corresponding to that with maximal correlation coefficient.
...
Overall, the average is close yet higher to the (hidden) ground truth value used to generate the sequence
(with $h_A = XX \pm XX$ \CP{a calculer !} for the aSPEM session and
$h_R = XX~\ms \pm XX$ \CP{a calculer !} for the rating scale),
and we observe that this value is higher for aSPEM compared to rating scale.
Quantitatively, the ratio of both hazard rates computed over participants was \CP{with a ratio $XX~ \pm XX~$ a calculer !},
...
but the dataset is still insufficiently large to shows the significativity of such observation
...
In particular, we also observed the same behavior for each sub-block individually,
showing that the origin of this variability mainly comes form inter-subject variability.
...
Such an analysis suggests that even if the predictive processes
at work in both sessions may explain a common origin for the evaluation of volatility,
this value is then modulated by individual preferences
...
short in comparison to the several hundreds
of trials that are commonly used in psychophysics and sensorimotor adaptation studies and that in that study, 
the role of environmental volatility was not addressed at all.
...
but would consist in a natural extension of it
...
\item Finally, we found that the experimental data of each different participant matched to different levels of belief's precision on the volatile environment, some being more or less conservative. Interestingly, each of the two experiments (namely for the unconscious anticipatory eye movements and the conscious rating) provided different distributions, opening the perspective for future \emph{computational phenotyping} using such a volatile setting.
\end{itemize}
%: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
