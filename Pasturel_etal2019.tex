...
\AM{THIS SEEMS TO BE TRUE ON AVERAGE, NOT AT EACH TRIAL... Note that $\beta^{(0)}_t=h$ and thus $B=\sum_{r \geq 0} \beta^{(r)}_{t-1} \cdot \pi^{(r)}_{t}$.}
...
\AM{IS THE FOLLOWING SENTENCE NECESSARY? Note that the series defined by $\mu^{(r+1)}_{r+t}$ is the moving average at trial $r+t$ on the $r$ last
samples.}
