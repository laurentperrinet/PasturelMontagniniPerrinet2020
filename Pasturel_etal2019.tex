...
we have computed the likelihood at each node of each alternative outcome
(the two hypothesis $o=1$ or $o=0$)
knowing the mean bias $\mu$ 
and conditioned on the run-length $r$. 
For each outcome, the likelihood of observing an occurrence of $o$ at the next trial,
is that of a binomial process knowing an updated sample size of $\nu+1$
and an updated estimation of the probability bias of $\mu' = \frac{\mu \cdot \nu + o}{\nu+1}$.
This defines the likelihood as :
\eql{
