...
itself generating sub-blocks with constant direction probability %between two switches separated by a random length
...
This provides with that the likelihood as $\pi^{(r)}_t = \Ll( \mu^{(r)}_{t-1}, \nu^{(r)}_{t-1} | x_0^{t})$.
...
%}
This probability $\hat{x_1}^t=Pr(x_0^{t+1} | x_0^{0:t})$ is computed as the expected value over all run-lengths:
\eql{
\hat{x_1}^t = \sum_{r} \mu^{(r)}_{t} \cdot \beta^{(r)}_{t}
\label{eq:readout}
}
...
we set nodes' beliefs to the constant vector $\beta^{(r+1)}_t = h \cdot r \cdot (1 -h) ^r$, 
...
%%%
%%%
\subsection{What statistics is learned in statistical learning?}
