...
\newcommand{\Abstract}{
Animal behavior has to constantly adapt to changes, for instance when the state of an environmental context changes unexpectedly. For an agent that interacts with this type of volatile setting, it is important to respond accurately and as quickly as possible. However, this procedure must generally be carried out in the presence of noisy sensory inputs and only on the basis of the information that is currently available. For instance, it has already been demonstrated that when a random sequence of right or left directions of motion of a visual target is biased, human observers can accurately anticipate it with their eye movements. Here, we have proved that this ability of observers to anticipate extends to a volatile where the probability biases could change at some randomly occurring switching times. In addition, we also recorded the level of confidence reported by human observers. These results were compared to that of a probabilistic agent which is optimal with respect to the generative switching model of events. We found a better match between the behaviorally observed response with that given by this agent compared to other models such as the leaky integrator. In addition, we could also fit experimental data with different prior levels of volatility in the model and deduce a common marker for inter-variability of participants, titrating their level of preference between exploration and exploitation. Such results prove that in such a volatile environment human observers may still effectively represent an internal belief, and use this representation for sensorimotor control and for explicit judgments. This work proposes an innovative approach to more generically test human cognitive abilities in uncertain and dynamic environments.
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
and which defines a new infection rate of this outbreak
but also an updated value of volatility (as given by the probability of a new outbreak)
at a longer time scale.
Alternatively, these cases are
``unlucky'' coincidences that originate from the variability
of the process which drives patients to the doctor
and which are in a shorter term 
an instance of values drawn from a stationary random process.
...
to introduce latent variables which formalize how different hypothesis predict these measurements.
...
representing prior knowledge about its structure.
...
Indeed, humans are able to accurately track a moving object
...
To begin with, we define a first ideal observer as a control, the \textit{forgetful agent},
which assumes that volatility is stationary with a fixed frequency
and that one can consider that volatility as the average probability of change at each trial.
...
may be fitted to variations of the experimental data across the sequence, 
...
as the weight decay correspond to assume \emph{a priori} a constant precision 
in the data sequence, contrary to more elaborate Bayesian models~\citep{Vilares2011}.
...
representative of a sub-part of our experimental sequence (see~\seeFig{intro}-A), % and~\seeFig{results_raw}),
...
Note that, while it takes some trials to detect switchs,
...
This number, that is called the \emph{run-length}, is useful in two manners.
...
\text{if} \quad x_2^t=0 \quad r^t = r^{t-1} +1 }\label{eq:run_length}}%see~\seeEq{run_length}
...
we will define the distribution which represents our belief
...
as an prediction / update cycle.
First, knowing this probability strength, % an estimate for our belief on the different variables at the previous trial $t-1$,
...
we show \AM{the eye velocity traces of} two representative subjects in~\seeFig{results_psycho}.
...
in the bottom panels, we compare the evolution of the recorded aSPEM (black line) with
the ground truth value of the (hidden) probability bias $x_1$
and the value inferred using the BBCP model, as well as the forgetful agent model (see~\seeFig{results_psycho},
as a green line, along with their respective confidence intervals in the shaded areas).
