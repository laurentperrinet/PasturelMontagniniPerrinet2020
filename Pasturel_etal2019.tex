...
As a comparison we have plotted in orange color the KDE computed over
...
We also estimated the mutual information %$\Cc$ (\seeEq{MI}) OR RATHER $MI$, as defined by ... NEW EQUATION}
...
and much better than that estimated for the leaky-integrator model ($r = XXX \pm $).
...
with the probability bias given by the model, as quantified by the average  mutual information ($r = XXX \pm $).
Importantly, this value is again higher for the BBCP model than
for the leaky integrator ($r = XXX \pm $).
