...
with $B$ such that $\sum_{r \geq 0} \beta^{(r)}_{t} = 1$. Note that $\beta^{(0)}_t=h$ and thus $B=\sum_{r \geq 0} \beta^{(r)}_{t-1} \cdot \pi^{(r)}_{t}$.
...
and where this binary function $\Ll(p, r | o)$ evaluates at each node the likelihood 
of each counterfactual alternative outcome $o=1$ or $o=0$
