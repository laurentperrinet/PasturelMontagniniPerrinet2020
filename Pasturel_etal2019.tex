...
\newcommand{\Abstract}{
Animal behavior has to constantly adapt to changes, for instance when the state of an environmental context changes unexpectedly. For an agent that interacts with this type of volatile setting, it is important to respond to such switches accurately and as quickly as possible. However, this operation should generally be performed in presence of noisy sensory inputs and only on the basis of currently available information. It has already been demonstrated that when a random sequence of right or left directions of motion of a visual target is biased, human observers can accurately anticipate it with their eye movements. Here, we have tested for the ability of these observers to anticipate different random probability biases while these could change at some randomly occurring times. The experimental results were compared to those of a probabilistic agent which is optimal with respect to this switching model. We found a better match between the behaviorally observed anticipatory response with that given by this agent compared to other models such as the leaky integrator. In addition, we could similarly fit the level of confidence reported by human observers with that provided by the model and deduce a common marker for inter-variability of participants, titrating their level of preference between exploration and exploitation. Such results prove that in such a volatile environment human observers may still effectively represent an internal belief, and use this representation for sensorimotor control and for explicit judgments. This work proposes an innovative approach to more generically test human cognitive abilities in uncertain and dynamic environments.
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
