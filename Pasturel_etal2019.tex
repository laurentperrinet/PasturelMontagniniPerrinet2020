...
~\textbf{(A)} This plot shows a synthesized sequence of $13$ events,
%each of them corresponding to a binary choice,
either a leftward or rightward movement of the target (TD, fine black lines).
A run-length is the hypothesis on the length of a sub-block over which the probability bias was constant,
that is, the number of trials since a switch.
Here, the true probability bias switched from a value of $.5$ to $.9$ at trial $7$,
as can be seen by the blue line showing the trajectory of the true run-length.
...
The probability for each node is represented by gray colors (black denoting sure)
and is shown in the inset for two representative trials: $5$ and $11$.
This shows how the model correctly integrated information to correctly identify a switch
and produce a prediction for the next trial $t=14$.
...
along with the (hidden) dynamics of the true probability bias $x_1$ (blue line),
...
We therefore draw a computational graph where for at any trial
...
Indeed, we know there is at any trial, there is a single true value for this variable
and that it thus represents a probability distribution over the run-length $r$. % (given the data).
...
knowing the true probability - isn't H the joint entropy and C something related to the mutual information? H and C should both be defines},
...
compared to the hidden true probability $x_1^t$.
...
of the probability bias $P_{\mathrm{true}}$ (blue line)
...
with the true probability and with the estimate obtained with the leaky integrator
...
As a comparison, the same method was applied to the true value $P_{\mathrm{true}}$ and 
...
($P_{\text{BBCP}}$: $r = 0.6678 \pm 0.0782 $ $MI =  0.7123 \pm 0.1250$, $P_{\text{leaky}}$: $r = 0.5480 \pm 0.0749 $ with $p=0.0037$ and $MI =  0.5777 \pm 0.0961$ with $p=0.0037$ ; $P_{\text{true}}$ : $r = 0.6138 \pm 0.0698 $ $p=0.0028$ and  $MI =  0.5622 \pm 0.1079$ with $p=0.0022$) 
and the bet experiment 
($P_{\text{BBCP}}$: $r = 0.8027 \pm 0.0905 $ $MI =  1.2557 \pm 0.3498$, $P_{\text{leaky}}$: $r = 0.6413 \pm 0.1209$ $p=0.0028$ and $MI =  0.9666 \pm 0.3008$ with $p=0.0022$ ; $P_{\text{true}}$ : $r = 0.6949 \pm 0.0861 $ with $p=0.0022$ and $MI =  0.9400 \pm 0.2550$ with $p=0.0022$).
...
with respectively $\text{MI} = XX~ \pm XX~$
...
quand on prend un r par sujet il semble que ce soit un peut plus grand que quand on garde un h fixe, pas pour le MI ni pour quand on calcule la moyenne des 3 r par sujet (notebook 5)}  for the first session and
$\text{MI} = XX \pm XX$
...
(with $h_A = XX \pm XX$ \CP{moyenne : $0.06692999125125686 \pm 0.03628253053391533$, mediane : 0.06904761904761905 	25\% 0.038461538461538464 	75\% 0.09318181818181817} for the aSPEM session and
$h_R = XX~\ms \pm XX$ \CP{moyenne : $0.06631464647744617 \pm 0.09970025861060718$, mediane : 0.02564102564102564 	25\% 0.011951909476661951 	75\% 0.09318181818181817
