...
\AM{Note that the series defined by $\mu^{(r+1)}_{t}$ is simply the average of the observations over the last $r$ samples.}
...
\AM{($P_{\text{BBCP}}$: $r = 0.8027 \pm 0.0905 $ $MI =  1.2557 \pm 0.3498$, $P_{\text{leaky}}$: $r = 0.6413 \pm 0.1209$ $p=0.0028$ and $MI =  0.9666 \pm 0.3008$ with $p=0.0022$ ; $P_{\text{true}}$ : $r = 0.6949 \pm 0.0861 $ with $p=0.0022$ and $MI =  0.9400 \pm 0.2550$ with $p=0.0022$).}
...
For \AM{$1600$ I THINK IT HAS TO BE $600$?} linearly spaced values of $\tau$ from $1$ to \AM{$1600$} trials,
...
We observe that these values are close to the (hidden) ground truth value ($h=1/40=0.025$) used to generate the sequence. In addition, the best-fit hazard rate value is higher for aSPEM compared to the rating scale measures.
...
when a more conscious cognitive process is at stake. \AM{HERE WE DO NOT COMMENT ON THE FACT THAT H-VA IS QUITE SYSTEMATICALLY OVERESTIMATED, WHEREAS H-BET IS LESS BIASED. WOULD THIS BE WORTH MENTIONING?}
...
are the result of two distinct readout processes
using the same internal representation of motion regularities.
\AM{Note that both these history lengths can be considered
short in comparison to the several hundreds
