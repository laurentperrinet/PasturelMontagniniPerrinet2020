...
For instance, it was already shown that human observers can accurately anticipate
a target's motion direction with their eye movements 
throughout random sequences of rightward/leftward motions
which were biased in direction probability.
Here, we tested the ability of these observers
to anticipate different random biases
within random-length contextual blocks.
Experimental results were compared to
those of a probabilistic agent which is optimal
...
compared to other models such as a leaky integrator model.
...
This work opens up a novel approach to more generically test
...
As such, two alternate strategies are open to us:
Either, there is an outbreak of measles and 
one should estimate its strength 
(as measured as the rate of new cases)
since the onset of the outbreak.
Alternatively, these cases are false positives:
they are coincidences that come from
the (possibly noisy and heterogeneous) process 
that generated these observations.
Then, a possibility is to readjust the baseline rate of infection
with this new data.
This example illustrates one basic problem with which our cognitive system is faced:
when observing new sensory evidence, 
should I stay and continue to integrate this novel data 
to my current beliefs about the environment's state
or should I go and adopt a new hypothesis 
...
such that one can act upon them at the present time~\citep{PerrinetAdamasFriston2014}.
...
\textbf{(A)}~We replicated the results of~\citep{Montagnini2010}, 
in which human observers were presented $600$ trials 
of target motions with a probabilistic bias. % TODO:check
Each trial consists of sequentially:
...
% TODO : dans notre plot on a des range de valeurs pour p :-/
...
Second, we generalized the generation of random sequences
by using random-length blocks
to avoid potential inter-blocks confounds (see \seeFig{results_raw}).
Third, to titrate this adaptive mechanism at the conscious level,
we asked each observer to perform the same experiment on a subsequent day,
and asked participants to rate the level of confidence
...
with contextual contingencies in shaping (oculomotor) behavior.
...
to minimize as fast as possible the behavioral impairment due
...
between target motion onset and foveation~\citep{ref}.
...
To further test such results to more generic experimental conditions,
it is necessary to extend the experimental protocol of~\citep{Montagnini2010} in three aspects.
...
we should use a continuous range of probability biases~$p$. 
...
The main conclusion from~\citet{Maus2015} was that
...
To study the effect of history length in all generality,
we should therefore extend the protocol such that the probability bias 
is fixed in sub-blocks of a large variety of block's length.
%The equations for this protocol will be detailed below~(\seeSec{bayesian_change_point}).
Last, it is crucial to generalize those results by repeating the same protocol
in a psychophysical judgement task in parallel to the recording of eye movements.
In such a task, we should evaluate for each participant their confidence for the next trial 
...
More specifically, the goal of this study is to generalize the adaptive process
...
as a function of trial order by making it itself a random variable. %,
...
one may generate trajectories for the value of $p$
...
below the trajectory of the underlying (hidden) bias' probability.
...
is  drawn from a Bernoulli process with parameter its frequency $1/\tau$.
...
There is always a switch at $t=0$ (that is, $x_2^0=1$) and for each pause.
...
To sum up, \seeEq{bsm} defines the Binomial Switching Model (BSM)
which we will use both for the generation of sequences in experiments
...
we will here provide with an analytical solution
...
which assumes that volatility  of the probabilistic bias occurs 
...
the temporal evolution for the expectation $p=x_1^t$ of a given event
...
The estimated  probability $\hat{x_1}^{t}$ is the integration of previous instances
...
It is thus a form of a moving average:
\eql{
...
Assuming independence of these occurrences, the probability is thus the sum
of the observations weighted by the beliefs $i$ trials in the past, that is,
...
We therefore proved here that the heuristic derived from~\citep{Anderson2006}
is an ideal inversion of the generative model 
which assumes a fixed drift in the probability bias.

%: using \hat{p} as a regressor
The correspondence that we proved between the moving average heuristic
and the ideal observer model as a solution to a generative model drives
us to several interim conclusions.
First, the trajectory $\hat{x_1}^{t}$ can serve as a regressor
...
We believe here that the challenge is not to validate if the brain would use Bayes' theorem,
...
along with the probability $Pr(x_2^t=1)$ of the occurrence of switches.
% ------------------------------------------------------------------
...
to only that samples $x_0^t$ produced since the switch and until $t$.
Indeed, those before the switch are drawn independently from the present true value $x_1^t$.
...
%-------------------------------------------------------------%
In order to define in all generality the detection model,
...
Using this latent variable, the goal of predictive coding is to 
...
for all different hypothesis as $\beta^{(r)}_t=Pr(r^t | x_0^{0:t-1})$.
...
First, we will \emph{update} beliefs on all nodes at trial $t$
...
we will make a \emph{prediction} for our belief of the state at the current trial $t$,
...
Mathematically, a belief on the probability bias $p$ defining the binomial trial
...
where this distribution is here parameterized by its sufficient statistics,
...
as the probability of the updated 
probability bias of $\frac{p\cdot r + o}{r+1}$
...
this ideal observer with a known ground truth before using it 
...
\centering{
\begin{tikzpicture}%[thick,scale=1, every node/.style={scale=1} ]
\node [anchor=north west]  (img4) at (0.000\linewidth,.618\linewidth){\includegraphics[width=0.49\linewidth]{4_A_result_psycho_velo}};
...
compare that value with that obtained by other models (see~\seeApp{bcp}).
...
Similarly to the strength of aSPEM, we find the following results:
(1) This trace exhibits a positive correlation:
the next outcome will in general be correctly inferred,
as compared to a random choice.
(2) A stronger bias leads to a lower variabilities in the estimate.
...
participants tended to provide with more neutral scales
...
are qualitatively similar to that which is unconsciously generated behaviorally
...
This exhibits a consistent relationship
...
More importantly, there is a slight but consistent
