...
\emph{Behavioral experiments: tracking eye movements and confidence rating}
\AM{I WOULD SKIP THIS SENTENCE: We replicated the results of~\citet{Montagnini2010},
...
a moving ring-shaped target (with $10\degree/s$ velocity) which the observers were instructed to follow.
...
always left for $p=0$ and always right if $p=1$. Participants' eye movement were recorded all along the trial sequence.
\AM{An important difference with the study of~\citet{Montagnini2010} is that their experiment was made of several $500$ trials blocks of target motion with a block-dependent direction probability bias selected among a few predetermined values, whereas in the present study $p$ can change at any moment during the sequence and take any value between $0$ and $1$.}
...
\textbf{(B)}~Horizontal eye velocity traces 
...
\textbf{(C)}\AM{I WOULD NOT WRITE ALL THIS IN THE FIGURE CAPTION: ~In this study, we extended this experiment in three aspects.
...
Third, } In order to titrate the adaptation to the environmental volatility of motion direction at the conscious level,
we invited each observer to perform a new variant of the direction-biased experiment on a subsequent day,
where, instead of recording their eye movements, we asked participants to rate, before each trial, the level of confidence
...
that the same sequence of directions was used in both experiments.
}
\label{fig:intro}
\end{figure}
...
\caption{
\emph{The binomial switching model to generate a trial sequence, and raw behavioral results.}%
 This diagram represents one instance of $200$ trials
as generated by the binomial switching model. %
We plot
on the top row the sequence of target directions (TD)
that were presented to observers.
TD is a binary random variable (right/left) which varies dynamically
as a function of the number of trials.
Below, we plot the (hidden) evolution of the underlying value 
of the probability bias $p$ used to generate the TD sequence (red shaded areas).
The dynamic of $p$ is piecewise constant and 
one can infer the underlying times for switches (illustrated by steps in the red-shaded areas).
%
Overlaid on this plot, we show  the trace of
raw results for two representative observers.
We have namely 
...
2/ the rating scale confidence judgments (red line).
...
In addition, the explicit predictive rating
for the next trial's motion direction
seems to follow a similar trend.
%
\AM{I WOULD MOVE THIS PART IN THE CAPTION OF FIG 1 OR IN THE TEXT : Globally, each of the $12$ participant see 3 blocks of $200$ trials,
...
we could present the exact same sequence to all participants.} %
...
we show
% the inferred probability of different run lengths at each trial and
% below, we superpose
...
% TODO: say something about convergence?
%-------------------------------------------------------------%
%: 2Cc perspectives
%-------------------------------------------------------------%
