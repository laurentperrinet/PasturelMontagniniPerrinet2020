...
This shows that
assuming that changes occur at a constant rate ($\hat{x}_2^t=h$)
...
(the inverse of variance) on the estimate $\hat{x}_1^{t+1}$
...
and the probability $\hat{x}_1^t$ is estimated  by integrating sensory evidence in this epoch (it 'stays').
...
with the inferred probability bias $\hat{x}_1$.
